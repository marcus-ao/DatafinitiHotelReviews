...
- 历史实验层：`E1-E10` 的旧结果已形成 archive 级材料，可用于对照和回溯
- 新执行工程层：面向“全量正式重跑 `E1-E10 + G1-G4`”的统一检索/生成/统计检验/Judge/盲评导出底座已经大体接通

当前真正所处阶段不是“继续沿用历史结果”，而是：

- 重新跑完 `E1-E10`
- 重新跑完 `G1-G4` 当前正式 decisive scope（`68` 条查询）下的统一矩阵结果；`q021 / q024` 已从 decisive matrix 中剔除并转为 supporting boundary cases
- 在 8 小时内完成一轮完整实验闭环
...
以下历史实验结果只作为 archive 参考保留，不直接作为本轮正式 canonical 结果：
...
- 但本轮正式 canonical 结果（`E1-E10 + G1-G4`）尚未全部重跑并登记到 registry
...
当前最重要的不是继续依赖历史实验，而是按正式执行协议：

1. 完整重跑 `E1-E10`
2. 完整重跑 `G1-G4`
3. 回填 registry / manifest
4. 完成统计检验、Judge、盲评和章节总报告
