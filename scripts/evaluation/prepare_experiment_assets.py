...
def build_frozen_config(cfg: dict) -> dict:
    return {
        ...
        "behavior": {
            "llm_backend": cfg["behavior"].get("llm_backend", "local"),
            "base_model": cfg["behavior"]["base_model"],
            "api_base_url": cfg["behavior"].get("api_base_url"),
            "api_key_env": cfg["behavior"].get("api_key_env", "OPENAI_API_KEY"),
            "enable_thinking": bool(cfg["behavior"].get("enable_thinking", False)),
            "temperature": float(cfg["behavior"].get("temperature", 0.0)),
            "max_new_tokens": int(cfg["behavior"].get("max_new_tokens", 256)),
            "api_timeout_seconds": int(cfg["behavior"].get("api_timeout_seconds", 120)),
