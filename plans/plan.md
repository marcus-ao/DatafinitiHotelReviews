...
- `09_validate.py` 在用户本地终端已通过 `28/28`
...
| 情感分布 | 与评分分布对比 | 已通过 |
| 端到端验收 | `09_validate.py` | 用户本地 28/28 通过 |
...
- 导入脚本：`scripts/08_load_to_postgres.py`
- 连接库：`psycopg2`
...
| 步骤 | 脚本 | 输入 | 输出 | 当前结果 |
| --- | --- | --- | --- | --- |
| 1 | `01_load_filter.py` | 原始 CSV | `city_filtered.pkl` | 6,171 |
| 2 | `02_clean_and_dedupe.py` | `city_filtered.pkl` | `cleaned_reviews.pkl` | 5,947 |
| 3 | `03_split_sentences.py` | `cleaned_reviews.pkl` | `sentences.pkl` | 51,813 |
| 4 | `04_classify_aspects.py` | `sentences.pkl` | `aspect_labels.pkl` | 63,085 |
| 5 | `05_classify_sentiment.py` | `aspect_labels.pkl` | `aspect_sentiment.pkl` | 63,085 |
| 6 | `06_build_profiles.py` | `aspect_sentiment.pkl` | `hotel_profiles.pkl` | 876 |
| 7 | `07_build_vector_index.py` | 句子/评论/标签 | `evidence_index.pkl` + ChromaDB | 51,813 |
| 8 | `08_load_to_postgres.py` | 全部中间产物 | PostgreSQL | 6 张核心表导入完成 |
| 9 | `09_validate.py` | 中间产物 + PG + Chroma | 验收报告 | 28/28 通过 |
...
| `04_classify_aspects.py` | 最慢，可能数小时 | 推荐在 Colab 或云端执行 |
| `05_classify_sentiment.py` | 十几分钟到更久 | 已实测约数分钟 |
| `06_build_profiles.py` | 秒级 | 秒级 |
| `07_build_vector_index.py` | 取决于编码速度 | GPU 下几分钟级 |
| `08`–`09` | 分钟级 | 推荐回本地执行 |
...
- [x] `09_validate.py` 在用户本地终端通过 `28/28`
